...
class TestSobelOperator(unittest.TestCase):
    """Simple dummy test for Sobel operator to verify autograder is working."""
    ...
    def test_dummy_test(self):
        ...
        try:
            ...
            result = filters.sobel_operator(test_img)
            
            # Check output type and structure
            self.assertIsInstance(result, tuple, "Output should be a tuple")
            self.assertEqual(len(result), 3, "Output should have 3 elements (Gx, Gy, grad_magnitude)")
            
            Gx, Gy, grad_magnitude = result
            
            # Check each output component
            for component in [Gx, Gy, grad_magnitude]:
